...
public class OrderProcessor implements IOrderManagementRepository {
    ...
    public List<Product> getOrderByUser(User user) {
        List<Product> products = new ArrayList<>();
        try (Connection connection = DBUtil.getDBConn()) {
            String query = "SELECT p.productId, p.productName, p.description, p.price, p.quantityInStock, p.type FROM orders o JOIN products p ON o.productId = p.productId WHERE o.userId = ?";
            try (PreparedStatement statement = connection.prepareStatement(query)) {
                statement.setInt(1, user.getUserId());
                ResultSet rs = statement.executeQuery();
                while (rs.next()) {
